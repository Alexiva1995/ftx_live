                            ...
                            @if(!$presentations->isEmpty())
                                <div class="m-1">
                                    @if (Auth::user()->rol_id == 2)
                                        @foreach($presentations as $presentation)
                                            <div class="row" style="padding-bottom: 10px;">
                                                <div class="col-10">
                                                    <a href="{{route ('download_resource_file', [$event->id, $presentation->id])}}" class="btn btn-danger btn-block" target="_blank">{{$presentation->title}}</a>
                                                </div>
                                                <div class="col-2">
                                    ...
                                    @else
                                        @foreach($presentations as $presentation)
                                            <ul class="list-group">
                                                <li style="list-style:none;"><a href="{{route ('download_resource_file', [$event->id, $presentation->id])}}" class="btn btn-danger btn-block" target="_blank">{{$presentation->title}}</a></li>
                                            </ul>
                                        @endforeach
                                    @endif
                                </div>
